...
export function AddPermissionDialog({ open, onOpenChange }: AddPermissionDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("permission_tags").insert({
        name,
        description,
        permissions,
      })
      ...
      toast({
        title: "Success",
        description: "Permission tag created successfully",
      })
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to create permission tag",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Create Permission Tag</DialogTitle>
          <DialogDescription>Define a new permission tag with specific access rights</DialogDescription>
        </DialogHeader>
